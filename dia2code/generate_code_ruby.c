...
void generate_code_ruby(batch *b) {
    ...
    while ( tmplist != NULL ) {
        ...
        if ( ! ( is_present(b->classes, tmplist->key->name) ^ b->mask ) ) {
            ...
            sprintf(outfilename, "%s/%s.class.rb", b->outdir, strtolower(tmplist->key->name));
						/* hack to get filename without ./ */
						char pretty_outfilename[90];

						sprintf(pretty_outfilename,"%s",strtolower(tmplist->key->name));
						/* if someone knows better way to do kill ./ from outfilename, please contact me */
						sprintf(pretty_outfilename, "%s.class.rb", pretty_outfilename);
						// fprintf(stderr, "pretty_outfilename: %s", pretty_outfilename);
						
						/* i have not much c-experience, so i know that's pretty buggy */
...
//													printf("We're in: %s, type: %s",parents->key->name,tmpname);
                        	...
                        	fprintf(outfile, "%s", parents->key->name);
													...
													fprintf(outfile, " # can't inheirit %s\n", parents->key->name);
												}
												}
...
// document parameters
                    while (parama != NULL) {
fprintf(outfile,"%s# * param %s %s %s\n", TABS, parama->key.type, parama->key.name, strlen(parama->key.comment) > 0 ? parama->key.comment : "" );
                        parama= parama->next;
                    }
                    ...
                    if (bracket_opened == 1) { fprintf(outfile, " )"); }
/*                    if ( umlo->key.attr.isabstract ) {
                        fprintf(outfile, ";\n");
                    } else {
*/
